...
test("the page is self-contained — a full document with inline style and no external asset", () => {
  const html = renderAnswer(answerWith({ facts: [FACT], visuals: [makeVisual({ kind: "stat", title: "Spend", factIds: ["spend"], body: { value: "$32k/mo" } })] }));
...
test("a stat visual renders its value and title", () => {
  const html = renderAnswer(answerWith({ facts: [FACT], visuals: [makeVisual({ kind: "stat", title: "Monthly spend", factIds: ["spend"], body: { value: "$32k / mo", note: "up 4%" } })] }));
...
test("a progress-bar renders a fill proportional to value/max", () => {
  const html = renderAnswer(answerWith({ facts: [FACT], visuals: [makeVisual({ kind: "progress-bar", title: "Conversion", factIds: ["spend"], body: { value: 25, max: 100, unit: "%" } })] }));
...
test("a verdict-card carries a verdict class the CSS can colour", () => {
  const html = renderAnswer(answerWith({ facts: [FACT], visuals: [makeVisual({ kind: "verdict-card", title: "Health", factIds: ["spend"], body: { verdict: "green", line: "on track" } })] }));
...
test("the refusals footer lists what the gate dropped, so nothing hides", () => {
  const html = renderAnswer(answerWith({
    facts: [FACT],
    visuals: [makeVisual({ kind: "stat", title: "Spend", factIds: ["spend"], body: { value: "$32k" } })],
...
test("every string is HTML-escaped — no markup from content reaches the page raw", () => {
  const evil = makeVisual({ kind: "stat", title: "<script>alert(1)</script>", factIds: ["spend"], body: { value: "<img src=x onerror=alert(2)>" } });
